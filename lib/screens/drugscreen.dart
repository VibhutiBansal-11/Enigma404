...
import 'package:socio/model/drugs.dart';
...
class _DrugScreenState extends State<DrugScreen> {
  ...
  Future<void> _refreshMedicines(BuildContext context) async {
    await Provider.of<Drugs>(context,listen: false).fetchDrugbyName(2,controller.text);
  }
  List<Medicine> med;
  void didChangeDependencies() {
    if (_isInit) {
      ...
      Provider.of<Drugs>(context).fetchDrugbyName(2,controller.text).then((_) {
        setState(() {
          _isLoading = false;
        });
      });
  ...
  Widget build(BuildContext context) {
    final drugData = Provider.of<Drugs>(context);
    final drugs = drugData.items;
